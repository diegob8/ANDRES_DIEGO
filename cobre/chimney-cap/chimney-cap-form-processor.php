<?php
// VERSIÓN MEJORADA CON DEBUGGING Y PRICE CALCULATOR - Funciona con .cl, .com, .net, etc.
...
// Función para calcular el precio del chimney cap
function calculateChimneyCapPrice($dimA, $dimB, $dimC, $dimD, $dimE, $dimF, $materialType, $edgeDetail, $holeDiameter) {
    ...
    $holeDiameter = floatval($holeDiameter ?: 0);
    
    // Calcular área en pies cuadrados (asumiendo que las dimensiones están en pulgadas)
    // Usando dimensiones exteriores para el cálculo de material
    $length = ($dimA + $dimC) / 12; // Convertir a pies
    $width = ($dimB + $dimD) / 12;   // Convertir a pies
    $area = $length * $width;
    ...
    $edgeFactors = [
        'Closed Hem' => 1.0,
        'Closed Hem with Kick' => 1.25,
        'Raw No Hem & No Kick' => 0.85
    ];
    
    // Obtener costo del material (default a copper si no está seleccionado)
    $materialCost = isset($materialCosts[$materialType]) ? $materialCosts[$materialType] : $materialCosts['16oz. Copper'];
    
    // Obtener factor del borde (default a 1.0 si no está seleccionado)
    $edgeFactor = isset($edgeFactors[$edgeDetail]) ? $edgeFactors[$edgeDetail] : 1.0;
    
    // Costo base de mano de obra
    $baseLaborCost = 75.00;
    
    // Factor de complejidad del tamaño (tapas más grandes requieren más precisión)
    $sizeComplexityFactor = max(1.0, $area * 0.1);
    
    // Factor de complejidad del agujero (agujeros más grandes son más complejos de cortar)
    $holeComplexityFactor = $holeDiameter > 8 ? 1.1 : 1.0;
    
    // Calcular precio final
    $materialPrice = $area * $materialCost;
    $laborPrice = $baseLaborCost * $sizeComplexityFactor;
    $totalPrice = ($materialPrice + $laborPrice) * $edgeFactor * $holeComplexityFactor;
...
try {
    ...
    // Otros campos
    $hole_diameter = isset($_POST['hole_diameter']) ? clean_input($_POST['hole_diameter']) : '';
    ...
    if (empty($hole_diameter)) $errors[] = "Hole diameter is required";
    ...
    $estimated_price = calculateChimneyCapPrice(
        $dimension_a, $dimension_b, $dimension_c, $dimension_d, 
        $dimension_e, $dimension_f, $material_type, $edge_detail, 
        floatval($hole_diameter)
    );

    // HTML para el negocio (con precio incluido)
    $email_body = "
    <html>
    <head>
        <title>Chimney Cap Quote Request</title>
        <style>
    ...
    <body>
        <div class='container'>
            <div class='header'>
                <h2>Chimney Cap Quote Request</h2>
            ...
            <div class='content'>
                ...
                <div class='field'>
                    <span class='label'>Hole Diameter:</span>
                    <span class='value'>" . $hole_diameter . " inches</span>
                </div>
                <div class='field'>
            ...
            <div class='footer'>
                <p>This message was sent from the Chimney Cap Quote Request form</p>
    ...
    // HTML para el cliente (usando el diseño de 1.php con precio incluido)
    $client_body = "
    <html>
    <head>
        <title>Chimney Cap Quote Request Confirmation</title>
        <style>
            body { 
    ...
    <body>
        <div class='email-container'>
            ...
            <div class='diagram-section'>
                <img src='https://toromecanicoeventos.cl/cobre/chimney-cap/Chimney-Cap.webp' alt='Chimney Cap Dimensions Diagram' style='max-width: 100%; height: auto; border-radius: 5px; display: block; margin: 0 auto;'>
            </div>
            ...
            <div class='content'>
                ...
                <div class='info-grid'>
                    ...
                    <div class='info-row'>
                        <span class='label'>Hole Diameter:</span>
                        <span class='value'>" . $hole_diameter . "</span>
                    </div>
                    <div class='info-row'>
                        <span class='label'>Edge Detail:</span>
                        <span class='value'>" . $edge_detail . "</span>
    ...
    $business_result = sendEmailWithDebug(
        $to_email, 
        'New Quote Request from ' . $first_name . ' ' . $last_name . ' - $' . number_format($estimated_price, 2), 
        $email_body, 
    ...
    // Log detallado
    $log_entry = date('Y-m-d H:i:s') . " - BUSINESS EMAIL\n";
    ...
    if ($business_result['final_result']) {
        ...
        $client_result = sendEmailWithDebug(
            $email, 
            'Thanks for your request - Copper & Metal Supply Inc.', 
            $client_body, 
            'Copper & Metal Supply Inc.'
        );
        
        // Log del cliente
        $client_log = date('Y-m-d H:i:s') . " - CLIENT EMAIL\n";
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Quote request sent successfully! Estimated price: $' . number_format($estimated_price, 2),
