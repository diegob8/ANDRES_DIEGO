<?php
// VERSIÓN ACTUALIZADA PARA DECK DOOR PAN - Campos actualizados según HTML
...
// Función para calcular el precio del deck door pan (actualizada)
function calculateDeckDoorPanPrice($dimA, $dimB, $dimC, $dimD, $dimE, $dimF, $dimG, $dimH, $materialType) {
    ...
    $dimF = floatval($dimF ?: 0);
    $dimG = floatval($dimG ?: 0);
    $dimH = floatval($dimH ?: 0);
    
    // Calcular área en pies cuadrados (asumiendo que las dimensiones están en pulgadas)
    // Para deck door pan, usamos las dimensiones principales A y H para el área base
    $length = ($dimA) / 12; // Convertir a pies
    $width = ($dimH) / 12;   // Convertir a pies
    $area = $length * $width;
    
    // Agregar área adicional basada en otras dimensiones
    $additionalArea = (($dimB * $dimC) + ($dimD * $dimE) + ($dimF * $dimG)) / 144; // Convertir pulgadas cuadradas a pies cuadrados
    $totalArea = $area + $additionalArea;
    
    // Costos de material por pie cuadrado para deck door pan
    $materialCosts = [
        '16oz. Copper' => 18.00,
        '20oz. Copper' => 22.00,
        '24ga. Galvanized' => 10.00,
        '24ga. Bondarized' => 12.00,
        '24ga. Stainless Steel' => 16.00,
        '26ga. Stainless Steel' => 14.00
    ];
    
    // Obtener costo de material (con default)
    $materialCost = isset($materialCosts[$materialType]) ? $materialCosts[$materialType] : $materialCosts['16oz. Copper'];
    
    // Costo base de mano de obra para deck door pan
    $baseLaborCost = 75.00;
    
    // Factor de complejidad del tamaño
    $sizeComplexityFactor = max(1.0, $totalArea * 0.12);
    
    // Factor de complejidad para deck door pan (menos complejo que saddle)
    $panComplexityFactor = 1.2;
    
    // Calcular precio final
    $materialPrice = $totalArea * $materialCost;
    $laborPrice = $baseLaborCost * $sizeComplexityFactor * $panComplexityFactor;
    $totalPrice = $materialPrice + $laborPrice;
    
    // Precio mínimo para deck door pan
    return max($totalPrice, 150.00);
}
...
try {
    ...
    // Dimensiones (8 dimensiones: A, B, C, D, E, F, G, H)
    ...
    $dimension_f = isset($_POST['dimension_f']) ? clean_input($_POST['dimension_f']) : '';
    $dimension_g = isset($_POST['dimension_g']) ? clean_input($_POST['dimension_g']) : '';
    $dimension_h = isset($_POST['dimension_h']) ? clean_input($_POST['dimension_h']) : '';
    ...
    $estimated_price = calculateDeckDoorPanPrice(
        $dimension_a, $dimension_b, $dimension_c, $dimension_d, 
        $dimension_e, $dimension_f, $dimension_g, $dimension_h, $material_type
    );
    ...
    <html>
    <head>
        <title>Deck Door Pan Quote Request</title>
        <style>
    ...
    <body>
        <div class='container'>
            <div class='header'>
                <h2>Deck Door Pan Quote Request</h2>
            ...
            <div class='content'>
                ...
                <div class='dimensions-section'>
                    ...
                    <div class='field'>
                        <span class='label'>Dimension F:</span>
                        <span class='value'>" . ($dimension_f ?: 'Not specified') . " inches</span>
                    </div>
                    <div class='field'>
                        <span class='label'>Dimension G:</span>
                        <span class='value'>" . ($dimension_g ?: 'Not specified') . " inches</span>
                    </div>
                    <div class='field'>
                        <span class='label'>Dimension H:</span>
                        <span class='value'>" . ($dimension_h ?: 'Not specified') . " inches</span>
                    </div>
                </div>
                
                <div class='specs-section'>
                    <h3>Pan Specifications</h3>
                    <div class='field'>
            ...
            <div class='footer'>
                <p>This message was sent from the Deck Door Pan Quote Request form</p>
    ...
    <html>
    <head>
        <title>Deck Door Pan Quote Request Confirmation</title>
        <style>
            body { 
    ...
    <body>
        <div class='email-container'>
            <div class='header'>
                <h1>Thanks For Your Pan Request</h1>
                <div style='font-size: 12px; margin-top: 5px;'>" . strtoupper($first_name) . "</div>
            </div>
            
            <div class='diagram-section'>
                <img src='https://toromecanicoeventos.cl/cobre/deck-door-pan/Deck-Door-Pan.webp' alt='Deck Door Pan Dimensions Diagram' style='max-width: 100%; height: auto; border-radius: 5px; display: block; margin: 0 auto;'>
            </div>
            ...
            <div class='content'>
                ...
                <div class='info-grid'>
                    ...
                    <div class='info-row'>
                        <span class='label'>Dimension for F:</span>
                        <span class='value'>" . ($dimension_f ?: 'Not specified') . "</span>
                    </div>
                    <div class='info-row'>
                        <span class='label'>Dimension for G:</span>
                        <span class='value'>" . ($dimension_g ?: 'Not specified') . "</span>
                    </div>
                    <div class='info-row'>
                        <span class='label'>Dimension for H:</span>
                        <span class='value'>" . ($dimension_h ?: 'Not specified') . "</span>
                    </div>
                    <div class='info-row'>
                        <span class='label'>Type of Material:</span>
                        <span class='value'>" . $material_type . "</span>
    ...
    $business_result = sendEmailWithDebug(
        $to_email, 
        'New Deck Door Pan Quote Request from ' . $first_name . ' ' . $last_name . ' - $' . number_format($estimated_price, 2), 
        $email_body, 
    ...
    // Log detallado
    $log_entry = date('Y-m-d H:i:s') . " - DECK DOOR PAN REQUEST\n";
    ...
    $log_entry .= "Material: " . $material_type . "\n";
    ...
    if ($business_result['final_result']) {
        ...
        $client_result = sendEmailWithDebug(
            $email, 
            'Thanks for your Deck Door Pan request - Copper & Metal Supply Inc.', 
            $client_body, 
            'Copper & Metal Supply Inc.'
        );
        
        // Log del cliente
        $client_log = date('Y-m-d H:i:s') . " - CLIENT PAN EMAIL\n";
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Deck Door Pan quote request sent successfully! Estimated price: $' . number_format($estimated_price, 2),
    ...
    $error_log = date('Y-m-d H:i:s') . " - PAN FATAL ERROR: " . $e->getMessage() . "\n";
