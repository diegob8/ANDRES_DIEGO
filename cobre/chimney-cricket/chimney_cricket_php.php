<?php
// VERSIÓN ACTUALIZADA PARA CHIMNEY CRICKET - 6 dimensiones, 2 selectores
...
// Función para calcular el precio del chimney cricket
function calculateChimneyCricketPrice($dimA, $dimB, $dimC, $dimD, $dimE, $dimF, $materialType, $roofPitch) {
    ...
    // Calcular área del cricket (forma triangular/piramidal)
    $baseLength = $dimA / 12; // Convertir a pies
    $baseWidth = $dimB / 12;
    $height = $dimC / 12;
    
    // Área base y lados
    $baseArea = $baseLength * $baseWidth;
    $sideArea = (($dimD + $dimE + $dimF) / 12) * $height * 0.5;
    ...
    $materialCosts = [
        '16oz. Copper' => 22.00,
        '20oz. Copper' => 27.00,
        '24ga. Galvanized' => 14.00,
        '24ga. Bondarized' => 16.00
    ];
    
    // Factores por pitch del techo (cricket es muy sensible al pitch)
    $pitchFactors = [
        '1/12 Pitch' => 1.1,
        '2/12 Pitch' => 1.15,
        '3/12 Pitch' => 1.2,
    ...
    $pitchFactor = isset($pitchFactors[$roofPitch]) ? $pitchFactors[$roofPitch] : 1.2;
    
    // Costo base de mano de obra para crickets (más complejo que saddles)
    $baseLaborCost = 120.00;
    
    // Factor de complejidad del tamaño
    $sizeComplexityFactor = max(1.0, $totalArea * 0.2);
    
    // Factor de complejidad inherente del cricket
    $cricketComplexityFactor = 1.6;
    
    // Calcular precio final
    $materialPrice = $totalArea * $materialCost;
    $laborPrice = $baseLaborCost * $sizeComplexityFactor * $cricketComplexityFactor;
    $basePrice = $materialPrice + $laborPrice;
    
    // Aplicar factor de pitch
    $totalPrice = $basePrice * $pitchFactor;
    
    // Precio mínimo para crickets
    return max($totalPrice, 250.00);
}
...
try {
    ...
    // Dimensiones (6 dimensiones: A, B, C, D, E, F)
    ...
    $estimated_price = calculateChimneyCricketPrice(
        $dimension_a, $dimension_b, $dimension_c, $dimension_d, 
        $dimension_e, $dimension_f, $material_type, $roof_pitch
    );

    // HTML para el negocio
    $email_body = "
    <html>
    <head>
        <title>Chimney Cricket Quote Request</title>
        <style>
    ...
    <body>
        <div class='container'>
            <div class='header'>
                <h2>Chimney Cricket Quote Request</h2>
            ...
            <div class='footer'>
                <p>This message was sent from the Chimney Cricket Quote Request form</p>
    ...
    <html>
    <head>
        <title>Chimney Cricket Quote Request Confirmation</title>
        <style>
            body { 
    ...
    <body>
        <div class='email-container'>
            <div class='header'>
                <h1>Thanks For Your Cricket Request</h1>
                <div style='font-size: 12px; margin-top: 5px;'>" . strtoupper($first_name) . "</div>
            </div>
            
            <div class='diagram-section'>
                <img src='https://toromecanicoeventos.cl/cobre/chimney-cricket/Chimney-Cricket.webp' alt='Chimney Cricket Dimensions Diagram' style='max-width: 100%; height: auto; border-radius: 5px; display: block; margin: 0 auto;'>
            </div>
    ...
    $business_result = sendEmailWithDebug(
        $to_email, 
        'New Chimney Cricket Quote Request from ' . $first_name . ' ' . $last_name . ' - $' . number_format($estimated_price, 2), 
        $email_body, 
    ...
    // Log detallado
    $log_entry = date('Y-m-d H:i:s') . " - CHIMNEY CRICKET REQUEST\n";
    ...
    $log_entry .= "Pitch: " . $roof_pitch . " - Material: " . $material_type . "\n";
    ...
    if ($business_result['final_result']) {
        ...
        $client_result = sendEmailWithDebug(
            $email, 
            'Thanks for your Chimney Cricket request - Copper & Metal Supply Inc.', 
            $client_body, 
            'Copper & Metal Supply Inc.'
        );
        
        // Log del cliente
        $client_log = date('Y-m-d H:i:s') . " - CLIENT CRICKET EMAIL\n";
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Chimney Cricket quote request sent successfully! Estimated price: $' . number_format($estimated_price, 2),
    ...
    $error_log = date('Y-m-d H:i:s') . " - CRICKET FATAL ERROR: " . $e->getMessage() . "\n";
